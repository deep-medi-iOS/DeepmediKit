        ...
        if let rect  = cropFaceRect,
           ...
           let lowerLipContour = face.contour(ofType: .lowerLipBottom) {
            ...
            if willCheckRealFace {
                if !isTimerRunning {
                    let (left, right) = antiSpoofingValidator.checkReal(face)
                    isLeftEyeReal  = left
...
//                collectionByteData(sampleBuffer: sampleBuffer)
    ...
    private func getMaskedImage(
        ...
        let flipped     = imageOrientationMapper.flipImage(picture) ?? picture
        let flippedPath = imageOrientationMapper.flipPathHorizontally(cgPath, in: picture.size)
...
//        UIGraphicsBeginImageContextWithOptions(picture.size, false, 1.0)
        ...
        // ✅ 핵심: UIImage.draw는 imageOrientationMapper(회전/미러)을 반영해서 그려줌
