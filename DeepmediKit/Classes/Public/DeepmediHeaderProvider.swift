...
public final class DeepmediHeaderProvider {
    public init() {}
    public enum HeaderError: Error {
        case messegae(String)
    }
    public typealias HeaderErr = HeaderError
    ...
    public func getHeader(
        ...
        guard let url = URL(string: urlString) else {
            throw DeepmediHeaderProvider.HeaderError.messegae("Invalid URL")
        }
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw DeepmediHeaderProvider.HeaderError.messegae("Invalid response")
        }
        
        guard (200..<300).contains(httpResponse.statusCode) else {
            throw DeepmediHeaderProvider.HeaderError.messegae("status code error: \(httpResponse.statusCode)")
        }
