...
//        public let exposureOffset: Float
    ...
    internal let measurementFileWriter = MeasurementFileWriter(), measurementState = MeasurementState(), lightingChangeDetector = LightingChangeDetector()
    
    internal let model       = FaceKitConfigurationStore.shared,
...
// MARK: Property 
    ...
    internal func saveMeasurementOutputs() {
        ...
        let measurementComplete      = measurementState.measurementComplete,
            ...
            frameFilePath            = measurementState.frameDataFilePath,
            accFilePath              = measurementState.accFilePath,
            gyroFilePath             = measurementState.gyroFilePath,
        ...
        dispatchTimer?.setEventHandler { [weak self] in
            ...
            if self.sigR.count == measurementDataCount {
                if let rgbPath = self.measurementFileWriter.make(
                   ...
                   let frameDataPath = self.measurementFileWriter.saveFrameCSV(
                    data: frameDataArr
                   ),
                   let accCSV = self.measurementFileWriter.saveSensorCSV(
                    fileName: "sensor_accelerometer.csv",
                    data: acc,
                    timestamp: \.ts, x: \.x, y: \.y, z: \.z
                   ),
                   let gyroCSV = self.measurementFileWriter.saveSensorCSV(
